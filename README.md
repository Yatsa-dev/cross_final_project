...
Наскрізний проєкт магістратури GoIt Neoversity. Проєкт пройшов повний шлях: макет у
Figma → базові компоненти → навігація → інтеграція REST API → розділення глобального
стану → оптимізація продуктивності → фінальне розширення функціоналу.
...
Ранкова черга в кавʼярні коштує 10–15 хвилин. Сам напій готується за три — решта часу
йде на очікування: постояти, замовити, дочекатись, розрахуватись. Для людини, яка бере
каву дорогою на роботу, це і є головна причина не зайти.

## Рішення

Замовлення і оплата відбуваються в застосунку, час самовивозу обирає користувач.
Приходиш у вказаний час — напій уже готовий, черга не потрібна.

Сценарій використання:

```
меню → налаштування напою → кошик → час і оплата → підтвердження з номером → забрати
```

Повторне замовлення коротше: історія → «Повторити» → оплата. Улюблені напої винесені
в окремий розділ, щоб не шукати їх у меню щоразу.

## Що змінилось у фінальній версії

Попередня версія вже мала меню, кошик і навігацію, але поводилась як демонстрація, а не
як продукт: дані зникали при перезапуску, а частина інтерфейсу нічого не робила.

| Зона | Було | Стало |
| --- | --- | --- |
| Стан застосунку | Кошик і тема жили в памʼяті, перезапуск усе скидав | Кошик, замовлення, улюблене й тема зберігаються |
| Замовлення | Історія — захардкоджений масив, оформлене замовлення зникало | Справжні замовлення, деталі, підтвердження отримання, повтор |
| Профіль | «6 з 8 стаканів» — фіксований текст | Бонусна картка рахує реальні стакани із замовлень |
| Адреса в шапці | Шеврон є, натискання нічого не робить | Відкриває екран закладу з картою |
| Улюблене | Не існувало | Сердечко на картці, окремий екран, збереження |
| Карта закладу | Не існувало | Інтерактивна карта з маршрутом і дзвінком |
...
Redux Toolkit 2 · Reanimated 4 · AsyncStorage 2.
...
Правило руху даних просте: екран знає про стор і про хуки, компонент — ні. Компонент
отримує дані пропсами і повідомляє про дії колбеком.
...
Критерій простий: якщо дані потрібні кільком незвʼязаним гілкам дерева **і** мають
похідні значення — це Redux. Якщо їх читають усі, а змінює одне місце — досить Context.
...
Короткий розбір проєкту на 12 слайдів: **[BrewGo_presentation.pdf](BrewGo_presentation.pdf)**
— ідея, аналіз попередньої версії, нові функції, керування станом, архітектура й результат.
